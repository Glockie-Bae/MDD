...
def compute_relative_power(eeg, fs=256, fmin=4, fmax=20, df=0.1):
    """
    eeg: [channels, time]
    return: [freq_bins]
    """
...
def read_multimodal_data(df, EEG3_ROOT, EEG128_ROOT):
    data = []
...
subj_inf = extract_common_rows_exact(EEG_128_rest_subj_inf, EEG_3_subj_inf)
print(subj_inf)

data = read_multimodal_data(subj_inf, EEG_3_data_dir, EEG_128_rest_data_dir)
np.savez('egg_dataset.npz', data)